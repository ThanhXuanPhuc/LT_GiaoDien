...
public class JNotepad extends JFrame {
    ...
    private void processEvent() {
        ...
        // Thêm sự kiện cho nút Page Setup
        ...
        itemFindPrevious.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String find = JOptionPane.showInputDialog(JNotepad.this, "Tìm:"); // Sửa ở đây
                if (find != null && !find.isEmpty()) {
                    String content = txtEditor.getText();
                    int index = content.lastIndexOf(find);
                    if (index != -1) {
                        txtEditor.setCaretPosition(index);
                        txtEditor.select(index, index + find.length());
                    } else {
                        JOptionPane.showMessageDialog(JNotepad.this, "Không tìm thấy văn bản"); // Sửa ở đây
                    }
                }
            }
        });
        ...
        itemSearch.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String searchText = JOptionPane.showInputDialog("Tìm kiếm:");
                if (searchText != null) {
                    try {
                        Desktop.getDesktop().browse(new URI("https://www.bing.com/search?q=" + searchText));
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(null, "Lỗi mở trình duyệt: " + ex.getMessage());
                    }
                }
            }
        });
        itemGoTo.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String lineNumberStr = JOptionPane.showInputDialog("Đi đến dòng:");
                try {
                    int lineNumber = Integer.parseInt(lineNumberStr);
                    int lineCount = txtEditor.getLineCount();
                    if (lineNumber > 0 && lineNumber <= lineCount) {
                        int startOffset = txtEditor.getLineStartOffset(lineNumber - 1);
                        txtEditor.setCaretPosition(startOffset);
                        txtEditor.select(startOffset, startOffset);
                    } else {
                        JOptionPane.showMessageDialog(null, "Số dòng không hợp lệ");
                    }
                } catch (Exception ex) {
        ...
        itemTimeDate.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String timeDate = java.time.LocalDateTime.now().toString();
                txtEditor.insert(timeDate, txtEditor.getCaretPosition());
            }
        });
        itemRestore.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                size = 20; // Đặt kích thước về 20
                txtEditor.setFont(new Font("Arial", Font.PLAIN, size));
            }
        });
        itemViewHelp.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                JOptionPane.showMessageDialog(null, "Hướng dẫn sử dụng:\n- Tạo file mới: Ctrl + N\n- Mở file: Ctrl + O\n- Lưu file: Ctrl + S\n- Tìm kiếm: Ctrl + F\n- ...");
            }
        });
        itemSendFeedback.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String feedback = JOptionPane.showInputDialog("Nhập phản hồi của bạn:");
                if (feedback != null) {
                    // Xử lý phản hồi ở đây (ví dụ: lưu vào file hoặc gửi qua email)
                    JOptionPane.showMessageDialog(null, "Cảm ơn bạn đã gửi phản hồi!");
                }
            }
        });
        itemaboutNotepad.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                JOptionPane.showMessageDialog(null, "Ứng dụng Notepad Demo\nPhiên bản 1.0\nNhà phát triển: ADMIN");
            }
        });
